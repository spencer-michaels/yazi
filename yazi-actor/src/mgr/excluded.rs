...
use yazi_parser::{mgr::ExcludedForm, spark::SparkKind};
...
impl Actor for Excluded {
	...
	fn act(cx: &mut Ctx, opt: Self::Form) -> Result<Data> {
		let state = opt.state.bool(cx.tab().pref.show_excluded);
	...
	fn hook(cx: &Ctx, _: &Self::Form) -> Option<SparkKind> {
		match cx.source() {
			Source::Ind => Some(SparkKind::IndExcluded),
			Source::Key => Some(SparkKind::KeyExcluded),
			_ => None,
		}
	}
}
